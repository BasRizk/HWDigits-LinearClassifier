...
def perceptron_predict(X_test, weights):
    ...
    for i in range(X_test_expanded.shape[0]):
        x_sample = X_test_expanded[i]
        predictions[i] =\
            activation_function(np.mat(weights.T)*np.mat(x_sample))
    return predictions
...
# TODO Calculating confusion matrices
if not(os.path.exists(confusion_dir_path)):
    os.makedirs(confusion_dir_path)
    for i in range(0, 10):
        confusion_matrix = create_confusion_matrix(all_predictions[i], T_test_labels)
